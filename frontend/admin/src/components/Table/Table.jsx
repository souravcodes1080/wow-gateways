          ...
          <div key={dayIndex} className="day-row">
            ...
            {Array.from({ length: rooms }).map((_, roomIndex) => {
              const isBooked = roomAvailabilityData.some(booking => {
                const checkIn = moment(booking.checkIn).date(dayIndex + 1).format('YYYY-MM-DD');
                const checkOut = moment(booking.checkOut).date(dayIndex + 1).format('YYYY-MM-DD');
                return checkIn === checkOut && booking.rooms.includes(roomIndex + 1);
              });
              return (
                <div
                  key={roomIndex}
